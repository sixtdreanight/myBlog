...
export function CyberpunkHomepage() {
  ...
  const goTo = useCallback((idx: number) => {
    ...
    window.dispatchEvent(new CustomEvent('kappa:pager-change', { detail: { index: clamped, total: TOTAL } }))
  ...
  useEffect(() => {
    let sy = 0
    const onS = (e: TouchEvent) => { if (e.touches.length === 1) sy = e.touches[0].clientY }
  ...
  const cardData = [
    { t: 'PROJECT', h: '项目集', d: '开源项目与创意实验', url: '/projects', accent: 'amber' as const },
    { t: 'ARCHIVE', h: '文章归档', d: '技术笔记与深度思考', url: '/archives', accent: 'green' as const },
  ...
  const accentBorder = { amber: 'rgba(255,183,0,0.2)', green: 'rgba(80,220,120,0.2)', hazard: 'rgba(255,100,20,0.2)' }
  const accentGlow = { amber: 'rgba(255,183,0,0.08)', green: 'rgba(80,220,120,0.06)', hazard: 'rgba(255,100,20,0.06)' }
    ...
    <div className="khp-homepage">
      ...
      <div className="khp-spectrum" aria-hidden="true">
        {Array.from({ length: 16 }).map((_, i) => (
          <div key={i} className="khp-spectrum-bar" style={{ height: `${18 + Math.random() * 22}px`, animationDelay: `${i * 0.06}s` }} />
        ))}
      </div>
      <EventLog />
        ...
        <div className="khp-ticker-track">
          {[...TICKER_ITEMS, ...TICKER_ITEMS].map((item, i) => (
            <div key={`${i}-${item}`} className="khp-ticker-item">
              ...
              <span className={item.startsWith('TEMP') || item.startsWith('LOAD') ? 'khp-ticker-warn' : ''}>{item}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <nav className="khp-dots">
        {Array.from({ length: TOTAL }).map((_, i) => (
          <button key={i} className={`khp-dot ${i === stagedIndex ? 'on' : ''}`} onClick={() => goTo(i)} aria-label={`Section ${i + 1}`}>
      ...
      {/* Page 0: Hero */}
      <section ref={el => { pagesRef.current[0] = el }} className={`khp-page ${stagedIndex === 0 ? 'on' : ''} ${activeIndex > 0 ? 'past' : ''}`} data-dir={activeIndex === 0 ? dir : 0}>
          ...
          <div className="khp-hero">
            ...
            <div className="khp-hero-main">
              ...
              <h1 className="khp-hero-title" data-selectable>
                <span className="khp-hero-char" style={{ animationDelay: '0s' }}>梦</span>
                <span className="khp-hero-char" style={{ animationDelay: '0.06s' }}>夜</span>
                <span className="khp-hero-char" style={{ animationDelay: '0.12s' }}>的</span>
                <span className="khp-hero-char" style={{ animationDelay: '0.18s' }}>工</span>
                <span className="khp-hero-char" style={{ animationDelay: '0.24s' }}>厂</span>
              </h1>
              ...
              <a href="/archives" className="khp-btn-hud" data-selectable-highlight>
                <span className="khp-prompt">C:\&gt;</span> 检索完整观测日志 <span className="khp-btag">[ ARCHIVE ]</span>
              </a>
            </div>
  ...
  └──────┬───────────────┘
   ...
   └───────────────┘`}</pre>
      ...
      <section ref={el => { pagesRef.current[1] = el }} className={`khp-page ${stagedIndex === 1 ? 'on' : ''} ${activeIndex > 1 ? 'past' : ''}`} data-dir={activeIndex === 1 ? dir : 0}>
        <div className="khp-page-scroll">
          <div className="khp-nodes">
            <h2 className="khp-sh khp-s0" data-selectable><span className="khp-sh-sl">//</span> NODES</h2>
            <p className="khp-ss khp-s1">极致源于梦想，追求源于热爱。</p>
            <div className="khp-cards">
              {cardData.map((c, i) => (
                <a key={c.t} href={c.url}
                ...
                >
                  <div className="khp-cr khp-cr--tl" /><div className="khp-cr khp-cr--tr" />
                  <div className="khp-cr khp-cr--bl" /><div className="khp-cr khp-cr--br" />
                  ...
                  <span className="khp-card-cta"><span className="khp-prompt">C:\&gt;</span> {c.h === '项目集' ? '查看项目' : c.h === '文章归档' ? '浏览文章' : '访问友链'}</span>
                </a>
              ))}
            </div>
          </div>
      ...
      {/* Page 2: Tech + Snake */}
      <section ref={el => { pagesRef.current[2] = el }} className={`khp-page ${stagedIndex === 2 ? 'on' : ''} ${activeIndex > 2 ? 'past' : ''}`} data-dir={activeIndex === 2 ? dir : 0}>
        <div className="khp-page-scroll">
          <CyberpunkSnakeBg active={activeIndex === 2} />
          <div className="khp-tech">
            <h2 className="khp-sh khp-s0" data-selectable><span className="khp-sh-sl">//</span> TECH STACK</h2>
      ...
      {/* Page 3: Signals */}
      <section ref={el => { pagesRef.current[3] = el }} className={`khp-page ${stagedIndex === 3 ? 'on' : ''}`} data-dir={activeIndex === 3 ? dir : 0}>
        <div className="khp-page-scroll">
          <div className="khp-signals">
            <h2 className="khp-sh khp-s0" data-selectable><span className="khp-sh-sl">//</span> SIGNAL LOG</h2>
            <p className="khp-ss khp-s1">接收最新的信号脉冲</p>
            <div className="khp-signal-grid">
              {signalData.map((s, i) => (
                <a key={s.t} href={s.u} className={`khp-card-foil khp-signal khp-s${i + 2}`} data-selectable-highlight>
                  <div className="khp-cr khp-cr--tl" /><div className="khp-cr khp-cr--tr" />
                  <div className="khp-cr khp-cr--bl" /><div className="khp-cr khp-cr--br" />
                  <div className="khp-rivet-col" />
      ...
      {/* Page 4: Connect */}
      <section ref={el => { pagesRef.current[4] = el }} className={`khp-page ${stagedIndex === 4 ? 'on' : ''}`} data-dir={activeIndex === 4 ? dir : 0}>
        <div className="khp-page-scroll">
          <div className="khp-signals">
            <h2 className="khp-sh khp-s0" data-selectable><span className="khp-sh-sl">//</span> CONNECT</h2>
            <p className="khp-ss khp-s1">在以下频率找到我</p>
            <div className="khp-signal-grid">
              <a href="https://github.com/sixtdreanight" target="_blank" rel="noopener" className="khp-card-foil khp-signal khp-s2" data-selectable-highlight>
                <div className="khp-cr khp-cr--tl" /><div className="khp-cr khp-cr--tr" />
                <div className="khp-cr khp-cr--bl" /><div className="khp-cr khp-cr--br" />
              ...
              <a href="https://twitter.com/sixtdreanight" target="_blank" rel="noopener" className="khp-card-foil khp-signal khp-s3" data-selectable-highlight>
                <div className="khp-cr khp-cr--tl" /><div className="khp-cr khp-cr--tr" />
                <div className="khp-cr khp-cr--bl" /><div className="khp-cr khp-cr--br" />
              ...
              <a href="https://space.bilibili.com/514345038" target="_blank" rel="noopener" className="khp-card-foil khp-signal khp-s4" data-selectable-highlight>
                <div className="khp-cr khp-cr--tl" /><div className="khp-cr khp-cr--tr" />
                <div className="khp-cr khp-cr--bl" /><div className="khp-cr khp-cr--br" />
